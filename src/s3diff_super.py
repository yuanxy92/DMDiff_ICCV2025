...
class OPT_S3Diff(torch.nn.Module):
    ...
    def forward(self, c_t, deg_score, prompt_neg,prompt_pos=None):
        ...
        # print(deg_proj.shape)
        # deg_proj = torch.cat([torch.sin(deg_proj), torch.cos(deg_proj)], dim=-1)
        # print(deg_proj.shape)
        # deg_proj = torch.cat([deg_proj[:, 0], deg_proj[:, 1]], dim=-1)
        # print(deg_proj.shape)
        # deg_score = deg_score.unsqueeze(1) #B 1 p p 
        
        # deg_score = self.conv_W(deg_score)
        
        # deg_score = deg_score.squeeze(1)
